...
//
    const recursionPattern = (int1, int2, current = int1, increasing = false, arr = []) => {
        // write your code here
        arr.push(current);
        if (current === -4) {
          increasing = true;
        }
        if (current === int1 && increasing) {
          return arr;
        }
        if (increasing) {
          return recursionPattern(int1, int2, current + int2, true, arr);
        }
        return recursionPattern(int1, int2, current - int2, false, arr);
    }
